...
- **Self-hosted fonts — Google Fonts CDN removed** — `static/index.html` referenced three Google Fonts `<link>` tags, causing page-load hangs and broken font rendering on air-gapped hospital networks. Downloaded Inter (latin + latin-ext) and JetBrains Mono (latin + latin-ext) as WOFF2 variable-font files (176 KB total, embedded in binary via `rust-embed`). `style.css` now declares `@font-face` rules pointing to `/fonts/*.woff2`. The Content-Security-Policy in `src/web.rs` is tightened to `font-src 'self'` (was `font-src https://fonts.gstatic.com`) and `style-src 'self' 'unsafe-inline'` (was `... https://fonts.googleapis.com`). No external requests required at runtime. (#142)
