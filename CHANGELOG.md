...
### Fixed
- **Diff tab button and health pills re-appear after CSP tightening** — the self-hosted-fonts PR (#171) moved `style="display:none"` attributes from HTML to CSS ID rules (`#tab-btn-diff { display: none }` etc.) so the CSP `style-src 'self'` restriction was met. However, the JS that showed those elements used `element.style.display = ''` (empty string), which removes the inline override and lets the CSS `display: none` rule win — keeping them permanently hidden. Fixed by introducing a `.hidden { display: none !important }` utility class in `style.css` and applying it to the 7 affected elements in `index.html`; all `style.display` assignments in `static/render.js` and `static/ws.js` are replaced with `classList.add/remove/toggle('hidden')`. Keeps all visibility logic out of the JS display-value strings. (#172)
...
- **Self-hosted fonts — Google Fonts CDN removed, CSP tightened to strict** — `static/index.html` referenced three Google Fonts `<link>` tags, causing page-load hangs and broken font rendering on air-gapped hospital networks. Downloaded Inter (latin + latin-ext) and JetBrains Mono (latin + latin-ext) as WOFF2 variable-font files (176 KB total, embedded in binary via `rust-embed`). `style.css` now declares `@font-face` rules with `font-display: swap` pointing to `/fonts/*.woff2`. The CSP in `src/web.rs` is fully tightened: `font-src 'self'` (was `https://fonts.gstatic.com`), `style-src 'self'` with `'unsafe-inline'` removed entirely — all initial `display:none` states moved to ID-scoped CSS rules; dynamic colors (source bar, chips) set via `element.style` in JS, which is script execution not an inline style attribute. Font files are served with `Cache-Control: public, max-age=31536000, immutable` and a sha256-based `ETag` header so browsers cache them for the lifetime of a binary build. (#142)
...
- **Rate-limit the all-bookmarked eviction warn!** — when the store hits the size/count limit but every candidate message is bookmarked, the eviction loop emitted a `warn!` log on every single eviction attempt (potentially thousands per second under load), flooding log files and log aggregators. The warn is now suppressed to at most once every 30 seconds using an `Arc<AtomicI64>` last-warned timestamp on `MessageStore`. (#132)
- **MSH-2 encoding-character validation in `parse_delimiters`** — a malformed message like `MSH|||||...` would set `component = '|'`, making the field separator identical to the component separator. All downstream field splits then produced garbage: wrong field counts, values bleeding into component slots, silently corrupt output. `parse_delimiters` now validates all four MSH-2 characters: each must be printable ASCII, none may equal the field separator, and all four must be distinct from each other. Validation failure returns a descriptive parse error rather than propagating garbage. Four unit tests added covering the field-sep-collision, non-printable-byte, duplicate-encoding-char, and non-default-valid-delimiter cases. (#129)
- **CLI snippet now uses `nc` instead of non-existent `mllp-send`** — the onboarding command in `static/index.html` referenced `mllp-send`, which is not a standard tool and is not shipped with the project. Replaced with a `printf | nc` one-liner that works on any stock Linux/macOS system without extra installs. The MLLP framing bytes (VT `0x0b`, FS `0x1c`, CR `0x0d`) are embedded via `printf` escape sequences. (#141)
- **XSS hardening — CSP header + inline event handler removal** — all 29 `innerHTML=` assignments that used user-derived HL7 data passed through `esc()`/`escAttr()` helpers, but the pattern was fragile: one missed escape on a future addition could inject arbitrary scripts via crafted MLLP content. Two complementary fixes applied: (1) a `Content-Security-Policy: default-src 'self'` response header (with `'unsafe-inline'` for `style-src` only, required for inline `style=` attributes) is now emitted on every HTTP response via an axum middleware in `src/web.rs` — any inline script blocked by the browser even if escape is missed; (2) all inline `onclick=`, `onchange=`, `onkeydown=`, `onkeypress=` handlers removed from both `static/index.html` (static buttons) and from `innerHTML` strings in `static/render.js` and `static/diff.js`. Static buttons are now wired via `addEventListener` in `app.js` init; dynamically-generated elements use `data-action` / `data-id` / `data-tag` / `data-source` attributes and are dispatched through the existing `document.addEventListener('click', handleInteraction)` delegation chain, extended with a `document.addEventListener('change', ...)` for checkboxes. (#137)
...
- **Refactor — `web.rs` WebSocket deduplication + typed stats** — the four `StoreEvent` match arms in `handle_ws` each repeated the same `serde_json::json! + socket.send + is_err` pattern; extracted to a `send_ws_event` helper. `get_stats` now serializes a `#[derive(Serialize)] StatsResponse` struct instead of a hand-written `json!` macro, preventing field name drift. (#119, #120)
- **Refactor — `store.rs` mutation helper** — `add_tag`, `remove_tag`, and `toggle_bookmark` shared identical write-lock / `Arc::make_mut` / summary-clone / broadcast boilerplate. Extracted to a private `mutate_message` method; each public method is now a thin wrapper. (#134)
- **Docs — `hl7/types.rs` field-level doc comments** — added `///` comments to all public fields of `Hl7Message`, `Hl7MessageSummary`, `Hl7Segment`, `Hl7Field`, and `Delimiters` explaining semantics, nullability, and the JSON contract with the frontend. (#150)
- **Fix — dynamic startup banner** — the ASCII-art banner used hardcoded column widths sized for a 5-character version string. Padding is now computed at runtime from the actual version string length, so the box renders correctly for any semver string. (#116)
- **CI — add `push: main` trigger and rustc-versioned cache key** — CI now runs on direct pushes to `main` (e.g. changelog amendments) in addition to PRs. The cargo cache key includes the rustc hash so toolchain upgrades automatically bust stale build artifacts. (#145)
- **CI — matrix build job replaces three near-identical jobs** — the three separate Windows/macOS/Linux release jobs collapsed into a single `matrix`-strategy job, reducing `build.yml` from ~120 lines to ~70 with identical behaviour. (#146)
- **Dependency cleanup — remove `rolling-file` crate, migrate file logging to `tracing-appender`** — both `tracing-appender` and `rolling-file` provided rolling file appenders; `rolling-file` (last published 2023, no longer maintained) was the odd one out. File logging now uses `tracing_appender::rolling::daily` for daily rotation. The previously-unused `max_size_mb` and `max_files` config fields are removed from `LoggingConfig`; `harux.toml` updated accordingly. The dead `tests/test_rolling.rs` (a `main()` entry point with no assertions, leftover from debugging) is deleted. (#143, #148)
- **Tooling — add `[lints.clippy]` table to `Cargo.toml`** — clippy warnings are now surfaced during `cargo build` and `cargo check` locally, not just in CI. CI still passes `-D warnings` to fail on them. (#149)
- **Config — `harux.toml` now shows effective defaults** — every setting is uncommented with its default value, making the file self-documenting. The `file =` logging option remains commented to keep file logging opt-in. (#151)
...
| Commit | Description |
|--------|-------------|
| [`f4f59db`](https://github.com/Pappet/harux/commit/f4f59db) | `perf(ui):` incremental counters, tick-row cutoff, source-legend fast path (#173, #174, #175) |
...
| Commit | Description |
|--------|-------------|
| [`c4bfc7f`](https://github.com/Pappet/harux/commit/c4bfc7f) | `fix(store):` accurate byte tracking + rate-limit all-bookmarked warn (#131, #132, #135) |
| [`0aa59ce`](https://github.com/Pappet/harux/commit/0aa59ce) | `fix(parser):` validate MSH-2 encoding chars in parse_delimiters (#129) |
| [`cc51002`](https://github.com/Pappet/harux/commit/cc51002) | `fix/chore:` nc snippet, drop rolling-file, lints table, harux.toml defaults (#141, #143, #148, #149, #151) |
| [`4c5947d`](https://github.com/Pappet/harux/commit/4c5947d) | `fix(security):` CSP header + remove inline event handlers (#137) |
